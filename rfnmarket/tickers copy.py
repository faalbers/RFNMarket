...
class Tickers():
    ...
    def getTimeSeriesPeriod(self, symbols, startDate, endDate, update=False):
        timeSeriesData = self.vdata.getData(['timeSeries'], keyValues=symbols, update=update)['timeSeries']
        if not 'chart' in timeSeriesData: return {}
        timeSeriesData = timeSeriesData['chart']

        timeseries = {}
        date_range = pd.date_range(startDate, endDate).date
        for symbol, tsData in timeSeriesData.items():
            df = pd.DataFrame(tsData).T
            df.sort_index(inplace=True)
            df.index = pd.to_datetime(df.index, unit='s').date
    ...
    def getClose(self, symbols, startDate, endDate, update=False):
        prices = self.getTimeSeries(symbols, update=update)
        date_range = pd.date_range(startDate, endDate).date
        df = pd.DataFrame(index=date_range)
        for symbol in symbols:
            df = df.join(prices[symbol]['close'])
            df.rename(columns={'close': symbol}, inplace=True)
        df.dropna(axis=0, how='all', inplace=True)
        return df
    
    def getAdjClose(self, symbols, startDate, endDate, update=False):
        prices = self.getTimeSeries(symbols, update=update)
        date_range = pd.date_range(startDate, endDate).date
        df = pd.DataFrame(index=date_range)
        for symbol in symbols:
            df = df.join(prices[symbol]['adjclose'])
            df.rename(columns={'adjclose': symbol}, inplace=True)
        df.dropna(axis=0, how='all', inplace=True)
        return df
    
    def getRiskFreeRate(self, startDate, update=False):
        startDate = datetime.strptime(startDate, '%Y-%m-%d').date()
        prices = self.getTimeSeries(['^IRX'], update=update)['^IRX']['close']
        return prices[startDate:].iloc[0]

    def getCloseOld(self, symbols, startDate, endDate, update=False):
