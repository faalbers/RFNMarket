...
class QuoteStatistics(Base):
    def __init__(self, symbols, updateMax=False):
        ...
        if timeExpired.days < 1:
            if len(notFoundSymbols) > 0:
                ...
                log.info('Dayly QuoteStatistics skipped')
                return

        modules = ['defaultKeyStatistics']

        log.info('Running geQuoteStatistics on %s symbols daily' % len(self.symbols))
        modules = list(set(modules).intersection(const.YAHOO_QUOTE_SUMMARY_MODULES))
        if len(modules) == 0:
            log.error('Yahoo.getQuoteStatistics: No valid modules selected. Use Yahoo.getQuoteProfileModules for valid ones.')
            return
    ...
    def pushAPIData(self, symbolIndex, symbolData):
        ...
        for db in self.databases:
            db.updateYahooQuoteStatistics(symbol, pushData)
